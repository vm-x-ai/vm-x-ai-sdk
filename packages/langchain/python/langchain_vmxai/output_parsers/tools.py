...
from vmxai import RequestMessageToolCall


def parse_tool_call(
    raw_tool_call: RequestMessageToolCall,
    *,
    partial: bool = False,
    strict: bool = False,
    return_id: bool = True,
...
class JsonOutputToolsParser(BaseCumulativeTransformOutputParser[Any]):
    ...
    def parse_result(self, result: List[Generation], *, partial: bool = False) -> Any:
        ...
        if isinstance(message, AIMessage) and message.tool_calls:
            ...
            try:
                raw_tool_calls: List[RequestMessageToolCall] = []
                for tool in message.additional_kwargs["tool_calls"]:
                    raw_tool_call = RequestMessageToolCall()
                    raw_tool_call.CopyFrom(tool)
...
class PydanticToolsParser(JsonOutputToolsParser):
    ...
    tools: List[TypeBaseModel] # type: ignore
    """The tools to parse."""
        ...
        for res in json_results:
            try:
                if not isinstance(res["args"], dict):
                    raise ValueError(f"Tool arguments must be specified as a dict, received: " f"{res['args']}")
